(function(jQuery){
	...
	this.init=function(options){
		...
		var $html='<div id="xPopupMask">';
			...
			+'	<div id="xPopupContainer" class="xPopupContainer" style="width:'+$opt.width+'px;height:'+ $opt.height+'px">'
		...
		if(!$('#xPopup').size()){
			$('body').prepend($html);
            $('html').css({'background-attachment':'fixed','background-image':'url(about:blank)'})
		};
		var $tmps='#xPopup';
		if($opt.mask){
		...
		$($tmps).show();
		$('#xPopupContainer').css({width: $opt.width+'px',height: $opt.height+'px'});
		$('#xPopupContent').css({width: $opt.width +'px', height: $opt.height - 28 +'px'});
		$('#xPopupTitle').html($opt.title);
		if($opt.url!=''){
			$opt.html='<iframe id="xPopupIframe" name="xPopupIframe" style="width:100%;height:99%" frameborder="0" scrolling="auto" src="'+$opt.url+'"></iframe>';
			$('#xPopupIframe').css({height: parseInt($opt.height-5) +'px'});
		};
	...
	this.xMove=function(){
		$bWidth = $(window).width() - $opt.width - 21;
		$bHeight = $(window).height() - $opt.height - 21;
		if($opt.left > $bWidth){
			$opt.left = $bWidth;
		}else if($opt.left < 0){
			$opt.left = 0;
